...
linux)
    ...
    fedora)
        custom_install_scripts+=(
            'VENTOY_LATEST_VERSION=$(curl -s https://api.github.com/repos/ventoy/Ventoy/releases/latest | jq -rc ".assets[] | select(.content_type == \"application/x-gzip\") | .browser_download_url")'

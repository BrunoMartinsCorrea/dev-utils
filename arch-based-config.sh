...
# PAMAC
sudo ssed -Ri 's/#(?=RemoveUnrequiredDeps|NoUpdateHideIcon|EnableAUR|CheckAURUpsudo pip install pipenv autopep8 pylint pgclidates|OnlyRmUninstalled|DownloadUpdates)//g' /etc/pamac.conf
sudo ssed -Ri 's/(?<=RefreshPeriod\s=\s)\d+/2/g' /etc/pamac.conf
# PAMAC
